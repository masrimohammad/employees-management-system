		...
		if(!empty($status)){
            ...
            if($query->num_rows > 0){
                ...
                if($query->num_rows > 0){
                        ...
                        $sql = "INSERT INTO attendance (employee_id, date, time_in, status) VALUES ('$id', '$date_now', '$time_now', '$logstatus')";
                        if($conn->query($sql)){
                            $echoMsg = 'Hi '.$row['firstname'].', You have timed in successfully! '. $time_now. ' You smell productive :)';
                        }
                        else{
                ...
                if($query->num_rows > 0){
                    ...
                    if($query->num_rows < 1){
                        ...
                        if($row['time_out'] != '00:00:00'){
                            ...
                            $sql = "UPDATE attendance SET time_out = '".$time_now."' WHERE id = '".$row['uid']."'";
                            if($conn->query($sql)){
                                $echoMsg = 'Hi '.$row['firstname'].', You have timed out successfully! '. $time_now. ' Have a nice day :)';
